...
// Function that gets sprint custom field Ids for a given JIRA key
// Returns: JIRA sprint custom field Ids (customfield_xxxxx) customFieldId = 10007
function getSprintCustomFieldId(jiraKey) {
    var sprintField;
...
async function getIssueDetails(jiraKey) {
    ...
    var sprintField = getSprintCustomFieldId(jiraKey);
    var fieldParams = `fields=key,summary,parent,${businessAreaField},${projectNameField},${sprintField},${epicLinkField},${epicNameField},${teamNameField}`;
...
async function getIssueWorklogs(jiraKey) {
    ...
    let cleanWorkLogs = workLogs.map((logEntry) => {
        ...
        var getWorklogCommentText = function (obj, key) {
            ...
            if (obj && obj.hasOwnProperty('content')) {
        ...
        if (logEntry && logEntry.hasOwnProperty('comment')) {
...
(async () => {
    ...
    for (const key of keys) {
        ...
        if (issueDetails[sprintField]) {
            let sprintValues = issueDetails[sprintField].map((val) => {
                return val.name;
            });
            let sprintNewValues = sprintValues
                .toString()
                .replace(/[\s]+([a - z])([A - Z])/g, '$1 $2');
            Object.assign(row, { sprint: sprintNewValues.split(',').join(', ') });
        }
